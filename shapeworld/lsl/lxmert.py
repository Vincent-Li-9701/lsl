...
class Lxmert(nn.Module):
    ...
    def gen_visual_pos(self, grid):
        visual_pos = [[i/grid, j/grid, 1/grid, 1/grid] for i in range(grid) for j in range(grid)]
    ...
    def forward(self, visual_feats, visual_pos=None, input_ids=None, attention_mask=None, setting=None, preprocess=False):
        if preprocess:
            ...
            visual_pos = self.gen_visual_pos(GRID).expand(original_shape[1], GRID ** 2, 4).reshape(-1, 4).cuda()
            combined_pos = self.visual_token_type(torch.tensor([0] * ((original_shape[1] - 1) * GRID ** 2) + [1] * GRID ** 2).cuda())
            combined_pos += visual_pos
            combined_pos = combined_pos.expand(original_shape[0], original_shape[1] * GRID ** 2, 4)
        ...
        out = torch.mean(out[:, -GRID ** 2 :], dim=1)
        concat_out = self.query_out_bn(out) # This is essentially doing column-wise normalization
